...
int setKurs() {
    Kurs *a = NULL, *b = NULL;
    char kursName[25];
    printf("%¬ведите название курсовой ");
    scanf("%s",kursName);
    b=(Kurs *)malloc(sizeof(Kurs));
    strcpy(b->kursName,kursName);
    printf("¬ведите тему курсовой ");
    scanf("%s",b->kursTheme);
    printf("¬ведите обьем курсовой ");
    scanf("%d",&(b->capacity));
    ...
    printf("¬ведите шифр групы студента ");
    scanf("%s",b->std->group);
    int j=0;
    while(1){
        if(b->infrm!=NULL){
        ...
        j++;
        char sour[25];
        printf("¬ведите источник информации ");
        scanf("%s",sour);
         if (sour[0]=='*') break;
         b->infrm=(Inf *)malloc(sizeof(Inf));
         strcpy(b->infrm->source, sour);
         printf("¬ведите информацию \n\n");
        scanf("%s",b->infrm->inf);
        b->infrm->nexti = NULL;
    }
    printf("»м€ студента %s", b->std->name);
    if (ks==NULL) ks=b;
    return 0;
}
